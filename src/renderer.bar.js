...
export class BarRenderer extends renderer.Renderer {
  constructor(ctx, tayberry, series) {
    ...
    this.onResizeHandle = this.tb.registerCallback(
      'onResize',
      this.updateBarWidths.bind(this)
    );
  ...
  updateBarWidths() {
    ...
    const plotArea = this.tb.options.swapAxes
      ? this.tb.plotArea.clone().swapXY()
      : this.tb.plotArea;
    ...
    for (let seriesIndex = 0; seriesIndex < seriesCount; seriesIndex++) {
      ...
      if (series.animationState) {
        ...
        if (series.animationState.subtype === 'width') {
          ...
          transColour.a = 0;
          if (isShow)
            rState.colour = Colour.createFromBlend(
              transColour,
              new Colour(series.colour),
              series.animationState.stage
            ).toString();
          else
            rState.colour = Colour.createFromBlend(
    ...
    for (let categoryIndex = 0; categoryIndex < categoryCount; categoryIndex++) {
      ...
      const categoryXEnd =
        plotArea.left + Math.floor((categoryIndex + 1) * categoryWidth);
      // FIXME: Need to map this.tb.options.barPlot.categorySpacing
      const barXStart =
        categoryXStart
        + Math.ceil((categoryWidth * this.tb.options.barPlot.categorySpacing) / 2);
      const barXEnd =
        categoryXEnd
        - Math.floor((categoryWidth * this.tb.options.barPlot.categorySpacing) / 2);
      ...
      for (let seriesIndex = 0; seriesIndex < seriesCount; seriesIndex++) {
        ...
        const rState = series.rState;
        const value =
          Tayberry.getDataValue(series.data[categoryIndex]) * rState.yMultiplier;

        let barWidth = Math.floor(
          rState.multiplier * Math.floor((barXEnd - barXStart) / totalBarsPerCategory)
        );

        const xStart =
          Math.floor(barXStart + runningBarWidth)
          + Math.ceil(
            series.xAxis.mapLogicalXOrYUnit(this.tb.options.barPlot.barPadding) / 2
          );
        const xEnd =
          Math.ceil(barXStart + runningBarWidth + barWidth)
          - Math.floor(
            series.xAxis.mapLogicalXOrYUnit(this.tb.options.barPlot.barPadding) / 2
          );
        const yTop = series.yAxis.getValueDisplacement(
          value + (value > 0 ? yRunningTotalPositive : yRunningTotalNegative)
        );
        const yBottom = isStacked
          ? value > 0
            ? yBottomPositive
            : yBottomNegative
          : yOrigin;
  ...
  drawPlot() {
    ...
    while ((bar = barEnumerator.next())) {
      if (
        bar.series.visible
        & (constants.visibilityState.visible | constants.visibilityState.transitioning)
      ) {
        this.ctx.fillStyle = bar.selected
          ? bar.series.rState.highlightColour
          : bar.series.rState.colour;
    ...
    if (this.tb.options.labels.enabled) {
      ...
      while ((bar = barEnumerator.next())) {
        if (
          ...
          this.drawLabel(
            bar.value,
            bar.series.yAxis.options.labelFormatter(bar.value),
            bar.rect
          );
        }
      }
  ...
  hitTest(x, y) {
    ...
    const categoryIndex = Math.floor(
      (categoryCount * ((isHorizontal ? y : x) - plotArea.left)) / plotArea.width
    );
    ...
    while ((bar = barEnumerator.next())) {
      ...
      if (bar.rect.containsPoint(x, y)) {
        ...
        const xDist = Math.min(
          Math.abs(x - bar.rect.left),
          Math.abs(x - bar.rect.right)
        );
        const yDist = Math.min(
          Math.abs(y - bar.rect.top),
          Math.abs(y - bar.rect.bottom)
        );
        ...
        matches.push({
          ...
          data: {
            ...
            value: Tayberry.getDataValue(
              this.series[bar.seriesIndex].data[bar.categoryIndex]
            ),
          },
        });
      }
    ...
    if (matches.length) {
      ...
      ret.found = true;
      ret.normalisedDistance =
        matches[0].distance + Math.sqrt(matches[0].data.rect.area);
...
export class BarEnumerator extends renderer.ByCategoryEnumerator {
  next() {
    ...
    if (this.categoryIndex < this.categoryCount) {
      const [xStart, yTop, xEnd, yBottom] = this.renderer.barPositions[
        this.categoryIndex
      ][this.seriesIndex];
