...
Tayberry.prototype.onAnimate = function (timestamp) {
  ...
  for (let index = this.pendingAnimations.length - 1; index >= 0; index--) {
    ...
    if (animation.onFrame) {
      animation.onFrame(
        animation.initialStage
          + Math.min(elapsed / animation.length, 1) * (1 - animation.initialStage)
      );
    }
...
Tayberry.prototype.setSeriesVisibility = function (
  series,
  visible,
  subtype,
  onCompletion
  ...
  series.visible = visible
    ...
    : constants.visibilityState.hidden;
...
Tayberry.prototype.toggleSeriesVisibility = function (series) {
  this.setSeriesVisibility(
    series,
    !(series.visible & constants.visibilityState.visible)
  );
};
