...
export class Axis {
  static create(tayberry, options, index, axisType, xYSwapped) {
    const isHorizontal =
      (axisType === 'x' && !xYSwapped) || (axisType === 'y' && xYSwapped);
    if (options.type === 'linear')
      return new LinearAxis(tayberry, index, options, axisType, isHorizontal);
    else return new CategorialAxis(tayberry, index, options, axisType, isHorizontal);
  }
  ...
  setPlacement() {
    const validAndSpecificPlacements = [
      'left',
      'right',
      'top',
      'bottom',
  ...
  maxLabelSize() {
    ...
    return Utils.reduce(ticks, Math.max, (x) =>
      tb.getTextWidth(this.options.labelFormatter(x.value), this.labelFont)
    );
  }

  mapLogicalXOrYUnit(x) {
    return this.isVertical
      ? this.tayberry.mapLogicalXUnit(x)
      : this.tayberry.mapLogicalYUnit(x);
  }
      ...
      if (this.isVertical) {
      ...
      } else {
        {
          ...
          for (let i = 0; i < ticks.length; i++) {
            let tick = ticks[i];
            const textWidth = tb.getTextWidth(
              this.options.labelFormatter(tick.value),
              this.labelFont
            );
        ...
        if (ticks.length) {
          const lastTick = ticks[ticks.length - 1];
          const textWidth = tb.getTextWidth(
            this.options.labelFormatter(lastTick.value),
            this.labelFont
          );
  ...
  drawTicksAndLabels(offsetRect) {
    ...
    const labelPaddingX = this.isVertical
      ? labelPadding * (this.isPlacedAtStart ? -1 : 1)
      : 0;
    const labelPaddingY = !this.isVertical
      ? labelPadding * (this.isPlacedAtStart ? -1 : 1)
      : 0;
    const fontHeight = tb.getFontHeight(this.options.font);
    ...
    tb.labelsCtx.textAlign = this.isVertical
      ? this.isPlacedAtStart
        ? 'right'
        : 'left'
      : 'center';
    tb.labelsCtx.textBaseline = this.isVertical
      ? 'middle'
      : this.isPlacedAtStart
      ? 'bottom'
      : 'top';
    ...
    this.enumerateTicks((tick) => {
      ...
      if (
        this.isVertical
        || ((typeof lastXEnds[row] === 'undefined' || xStart > lastXEnds[row] + 1)
          && xStart >= 0
          && xEnd < tb.labelsCanvas.width)
      ) {
        maxWidth = Math.max(maxWidth, textWidth);
        tb.labelsCtx.fillText(
          formattedValue,
          tick.x + labelPaddingX + xOffset,
          tick.y + labelPaddingY + rowOffset + yOffset
        );
        lastXEnds[row] = xEnd;
      }
    ...
    this.adjustOffsetRect(
      offsetRect,
      this.isVertical ? maxWidth + labelPadding : fontHeight + labelPadding
    );
  ...
  adjustOffsetRect(offsetRect, displacement) {
    offsetRect[this.startProperty] += this.isPlacedAtStart
      ? -displacement
      : displacement;
  }

  get startProperty() {
    if (this.isVertical) return this.isPlacedAtStart ? 'left' : 'right';
    else return this.isPlacedAtStart ? 'top' : 'bottom';
  }
...
class CategorialAxis extends Axis {
  enumerateTicks(callback) {
    ...
    for (let i = 0; i < categoryCount; i++) {
      ...
      let y = y1;
      if (this.isVertical) [x1, y1, x2, y2, x, y] = [y1, x1, y2, x2, y, x];
  ...
  get plotDisplacement() {
    return this.isVertical
      ? -this.tayberry.plotArea.height
      : this.tayberry.plotArea.width;
  }
  ...
  getValueDisplacement(value) {
    let ret =
      this.getOrigin()
      + (this.plotDisplacement * (value + 0.5)) / this.options.categories.length;
...
class LinearAxis extends Axis {
  updateFormatter() {
    if (!this.options.labelFormatter) {
      if (this.options.labelFormat === 'percentage') {
        this.options.labelFormatter = Utils.createPercentageFormatter(
          this.max - this.min,
          this.options.labelPrefix,
          this.options.labelSuffix
        );
      } else if (this.options.labelFormat === 'currency') {
        this.options.labelFormatter = Utils.createFixedNumberFormatter(
          this.max - this.min,
          this.options.labelPrefix || this.options.currencySymbol,
          this.options.labelSuffix
        );
      } else {
        this.options.labelFormatter = Utils.createAutoNumberFormatter(
          this.max - this.min,
          this.options.labelPrefix,
          this.options.labelSuffix
        );
  ...
  enumerateTicks(callback) {
    ...
    for (let yValue = this.tickStart; yValue <= this.tickEnd && this.tickStep; ) {
      ...
      yValue =
        this.tickStart
        + Math.round((yValue + this.tickStep - this.tickStart) / this.tickStep)
          * this.tickStep;
    }
  }
  ...
  static snapScaledValue(scaledStep) {
    ...
    else if (scaledStep < 2) scaledStep = 2;
    else if (scaledStep < 2.5) scaledStep = 2.5;
    else if (scaledStep < 5) scaledStep = 5;
    else scaledStep = 10;
    return scaledStep;
  }
  ...
  calculateExtent() {
    ...
    if (!overriddenStart || !overriddenEnd) {
      const [dataMin, dataMax] = this.isYAxis
        ? this.tayberry.getDataMinMax(this)
        : this.tayberry.getDataXMinMax(this);
    ...
    this.tickStart =
      this.options.tickStepValue && overriddenStart
        ? this.min
        : Math.floor(this.min / this.tickStep) * this.tickStep;
    this.tickEnd =
      this.options.tickStepValue && overriddenEnd
        ? this.max
        : Math.ceil(this.max / this.tickStep) * this.tickStep;
  ...
  get plotDisplacement() {
    return this.isVertical
      ? this.tayberry.plotArea.height - 1
      : -(this.tayberry.plotArea.width - 1);
  }
  ...
  getOrigin() {
    let ret =
      this.tayberry.plotArea[this.isVertical ? 'bottom' : 'left']
      - ((0 - this.min) * this.plotDisplacement) / (this.max - this.min);
  ...
  getValueDisplacement(value) {
    let ret =
      this.getOrigin() - (value * this.plotDisplacement) / (this.max - this.min);
    ret = this.isVertical ? Math.floor(ret) : Math.ceil(ret);
    return ret;
  }
  ...
  getCategoryLabel(index, totalCategories, isRange) {
    if (isRange) {
      ...
      return Utils.formatString('{0} \u2264 x < {1}', [
        this.options.labelFormatter(this.min + start * axisRange),
        this.options.labelFormatter(this.min + end * axisRange),
      ]);
    } else {
