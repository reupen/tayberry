...
export class Legend {
  ...
  adjustSize(plotArea, reset) {
    ...
    if (this.tb.options.legend.enabled) {
      ...
      const indicatorSize = this.tb.mapLogicalXUnit(
        this.tb.options.legend.indicatorSize
      );
      const textWidths = this.getItemTextWidths();
      const maxLegendItemWidth = textWidths.length
        ? Utils.reduce(textWidths, Math.max) + indicatorSize + smallPadding
        : 0;
      const isVertical =
        this.tb.options.legend.placement === 'left'
        || this.tb.options.legend.placement === 'right';
      ...
      switch (this.tb.options.legend.placement) {
        case 'bottom':
          calculatedSize =
            largePadding + indicatorSize * numRows + numRowGaps * smallPadding;
          plotArea.bottom -= calculatedSize - this.calculatedSize;
          this.yPos =
            this.canvas.height - indicatorSize * numRows - numRowGaps * smallPadding;
          break;
        case 'top':
          calculatedSize =
            largePadding + indicatorSize * numRows + numRowGaps * smallPadding;
    ...
    if (this.tb.options.legend.enabled) {
      ...
      for (let index = 0; index < legendMetrics.items.length; index++) {
        ...
        if (item.textWidth) {
          ...
          const highlighted =
            this.tb.selectedItem.type === 'legend'
            && this.tb.selectedItem.data.series === series;
          series.renderer.drawLegendIndicator(
            this.ctx,
            series,
            item.indicatorRect,
            highlighted
          ...
          if (!(series.visible & constants.visibilityState.visible))
            this.ctx.fillStyle = new Colour(this.ctx.fillStyle)
              .multiplyAlpha(this.tb.options.legend.hiddenAlphaMultiplier)
              .toString();
  ...
  getMeasurements() {
    ...
    if (this.tb.options.legend.enabled) {
      ...
      for (let lineIndex = 0; lineIndex < newLineIndices.length; lineIndex++) {
        ...
        let x = isHorizontal
          ? this.tb.plotArea.left + this.tb.plotArea.width / 2 - lineWidth / 2
          : this.xPos;
        ...
        for (let index = lineStart; index < lineEnd; index++) {
          ...
          if (item.textWidth > 0) {
            item.rect = new Rect(
              x,
              y,
              x + indicatorSize + smallPadding + item.textWidth,
              y + indicatorSize
            );
