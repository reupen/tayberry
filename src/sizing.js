...
Tayberry.prototype.calculatePlotArea = function () {
  ...
  if (this.options.title.text) {
    ...
    this.plotArea.top +=
      this.getFontHeight(this.options.title.font)
  ...
  for (let i = 0; i < MAX_AXIS_CALC_SIZE_ATTEMPTS; i++) {
    ...
    if (
      Utils.none(this.yAxes.map((e) => e.adjustSize(this.plotArea)))
      && Utils.none(this.xAxes.map((e) => e.adjustSize(this.plotArea)))
      && !this.legend.adjustSize(this.plotArea)
    )
      break;
  }
...
Tayberry.prototype.hitTest = function (x, y) {
  ...
  if (matches.length) {
    matches.sort(
      (a, b) => !a.found - !b.found || a.normalisedDistance - b.normalisedDistance
    );
    ret = matches[0];
  }
  return ret;
